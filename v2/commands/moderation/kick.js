module.exports = {
  ...
  code: `$channelSendMessage[$getVar[logChannel];{newEmbed:{title:Kick Log}{field:Staff:<@$authorID> ($authorID)}{field:User Kicked:<@$$findMember[$message[1];no]> ($findMember[$message[1];no])}{field:Reason:$if[$checkContains[$splitText[2];!r]==true;$splitText[2];No Reason Specified]}{field:Time & Date:$if[$charCount[$hour]==1;0$hour;$hour]:$minute:$second UTC $toLocaleUppercase[$month] $date $year}{field:Place of Occurrence:<#$channelID> ($channelID)}{color:$getVar[default]}{timestamp}}]

$title[1;KICKED!]
$description[1;***$userTag[$findMember[$message[1];no]]*** *has experienced the power of Falling Camellia.*]
$image[1;https://eca.astrookai.repl.co/cdn/media/kick.gif]
$color[1;$getVar[default]]

$kick[$findMember[$message[1];no];$guildID;$if[$checkContains[$splitText[2];!r]==true;$splitText[2];No Reason Specified]]
$splitText[$message;!r ]
$clear[1;$authorID]
...
$argsCheck[>0;{newEmbed:{author:ERR-02 Bad Request:https://eca.astrookai.repl.co/media/err.png}{description:$getVar[err02]}{field:Usage:$jsonRequest[https://eca.astrookai.repl.co/commands/kick;Usage]:yes}{color:$getVar[red]}}]
